...
```json
{
	...
	"fixtures": [{ "path": "tests/fixtures/sample-data.json", "purpose": "Test data" }]
}
```
...
```json
{
	"filtered_tests": {
		"scope": "src/auth",
		"matched_files": ["src/auth/service.test.ts", "src/auth/middleware.test.ts", "tests/integration/auth-flow.test.ts"],
		"test_count": 34,
...
```json
{
	...
	"helpers": [{ "path": "tests/helpers/factories.ts", "purpose": "Test data factories" }]
}
```
