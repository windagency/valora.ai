...
describe('User registration with email verification', () => {
	it('should complete full registration flow', async () => {
		...
		// Act - Verify
		const verifyResponse = await request(app).post('/auth/verify').send({ token });
...
describe('Token security', () => {
	...
	it('should rate limit verification attempts', async () => {
		const attempts = Array(10)
			.fill(null)
			.map(() => request(app).post('/auth/verify').send({ token: 'invalid' }));
...
See `data/templates/TEST_GENERATION_REQUIREMENTS.md` for detailed examples and patterns
