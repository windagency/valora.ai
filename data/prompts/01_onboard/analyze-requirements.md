...
**CRITICAL: Your response MUST be ONLY valid JSON. No markdown, no explanations, no prose. Just the JSON object.**

The output schema is enforced by the pipeline via `outputs:` frontmatter. Produce a JSON object with the following top-level keys:

- `requirement_analysis` — categorised requirements by domain with user stories, technical requirements, and acceptance criteria
- `complexity_estimate` — point estimates by priority and domain, sprint timeline, and risk factors
- `user_stories` — flat list of all user stories with requirement traceability
- `dependency_graph` — requirement IDs mapped to their prerequisite requirement IDs
- `complexity_map` — per-requirement complexity level, points, and risk rating
- `traceability_matrix` — bi-directional requirement traces, coverage summary, and identified gaps
