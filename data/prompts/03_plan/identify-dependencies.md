...
```json
{
	"validation": [
		{
			"entity": "User",
			"rules": ["Email must be valid format", "Email must be unique", "Verification token must be 32 characters"],
			"library": "zod"
		}
	]
}
...
```json
{
	"critical_path": ["DB migration", "User model", "AuthService logic", "API endpoints", "Frontend components", "Tests"],
