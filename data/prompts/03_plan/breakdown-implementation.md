	...
	"implementation_steps": [
		{
			...
			"actions": ["Import email validation helper", "Add email format check to userSchema", "Export updated validator"],
		...
		},
		{
			...
			"actions": ["Add test for valid email", "Add test for invalid email"],
...
```json
{
	...
	"effort_estimate": {
		...
		"confidence": "high",
		"assumptions": ["Simple change with no dependencies", "Validator pattern already exists"]
	}
}
```
...
```json
{
	...
	"files": [".env.example", ".env.staging", ".env.production", "README.md"],
...
```json
{
	...
	"validation": {
		...
		"technical": ["Unit tests cover all edge cases", "TypeScript types are correct", "No linting errors"],
		"security": ["Tokens use crypto.randomBytes()", "Token expiration enforced"]
	}
}
```
	...
	"scenarios": [
		{
			...
			"test_cases": ["Token is 32 characters", "Token is URL-safe", "Token is unique on each call"]
		},
		{
...
```json
{
	...
	"prerequisites": ["Code must be reverted first", "Verify no active verification flows"],
...
```json
{
	"implementation_steps": [
		...
		},
		{
			...
			"validation": {
				"functional": ["Migration runs successfully up and down", "Columns added with correct types and defaults"],
				"technical": ["Migration tested on staging", "Rollback migration tested"]
			},
			"dependencies": [],
			"rollback": {
	...
	"rollback_strategy": {
		...
		"rollback_triggers": ["Email delivery failure > 50%", "Existing user auth broken", "Signup conversion drop > 15%"],
