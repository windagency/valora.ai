...
Read the reference file at `documentation/developer-guide/modern-cli-toolkit/` for detailed flags, patterns, and examples for each tool. Only read the references relevant to the current task:
...
```bash
bash scripts/install-tools.sh [tool1] [tool2] ...
