...
```json
{
	...
	"affected_files": ["src/api/auth.ts", "src/services/email.service.ts", "src/models/user.model.ts"],
	"scope_type": "backend",
	"risk_level": "medium"
}
```
...
```json
{
	...
	"requirements_list": {
		...
		"business_rules": ["Tokens expire in 24 hours", "Unverified users have limited access"]
	},
	"acceptance_criteria": [
		{
			"id": "AC-001",
	...
	"user_workflows": {
		...
		"edge_cases": ["Multiple clicks on same link", "Invalid/tampered token", "Email service down"]
	},
	"integration_points": [
		{
			"type": "api",
