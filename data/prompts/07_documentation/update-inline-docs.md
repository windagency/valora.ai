 ...
 * ```
 */
export async function sendVerificationEmail(email: string, userId: string): Promise<VerificationTokenResult> {
	// Implementation
}
````
...
async function sendVerificationEmail(email: string, userId: string): Promise<VerificationTokenResult> {
	// Implementation
}
