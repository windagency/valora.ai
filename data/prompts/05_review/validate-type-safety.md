...
**Note**: Ensure the project's `tsconfig.json` excludes `data/`, `.valora/`, `.git/`, and `node_modules/` in its `exclude` array. If running without tsconfig, specify the project source directory explicitly rather than checking the root.
...
```bash
# TypeScript - check for 'any' usage in project source (exclude data/)
...
```json
{
	"type_errors": {
		...
		"commands_run": ["pnpm exec tsc --noEmit --strict"]
	},
	"contract_violations": {
		"api_mismatches": [
			{
