...
import { getLogger, getRuntimeDataDir } from '@windagency/valora-runtime';
import { mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import * as path from 'node:path';

import { MEMORY_PERSIST_DEBOUNCE_MS, MEMORY_STORE_VERSION } from './constants.js';
...
export class MemoryStore {
	...
	appendEntry(category: MemoryCategory, entry: MemoryEntry, _links?: unknown): Promise<void> {
		// Idempotent on `entry.id` — replace if present, push otherwise. Matches
		// the vault backend's overwrite semantics so the manager can safely
		// re-append an entry when persisting late-arriving fields (e.g. the
		// supersedes wikilink in `promote()`). The `_links` argument is part of
		// the port contract for graph-aware backends; this legacy JSON store
		// does not track edges, so it is intentionally ignored.
		const storeFile = this.load(category);
		const existingIdx = storeFile.entries.findIndex((e) => e.id === entry.id);
		if (existingIdx >= 0) {
			storeFile.entries[existingIdx] = entry;
		} else {
