...
vi.mock('utils/paths', async () => {
	const actual = await vi.importActual('utils/paths');
	return {
		...actual,
		getGlobalConfigDir: vi.fn(() => '/tmp/nonexistent-global-config'),
		getPackageDataDir: vi.fn(() => '/tmp/nonexistent-package-data'),
		getProjectConfigDir: vi.fn(() => null)
	};
});
...
describe('ConfigLoader - File Integration', () => {
	...
	beforeEach(async () => {
		...
		savedEnv = {
			...
			VALORA_CONFIG_PATH: process.env.VALORA_CONFIG_PATH,
			AI_CONFIG_PATH: process.env.AI_CONFIG_PATH
		};
		...
		delete process.env.VALORA_CONFIG_PATH;
		delete process.env.AI_CONFIG_PATH;

		// Create ConfigLoader with explicit path to test config
		configLoader = new ConfigLoader(path.join(tempDir, 'config.json'));
	});
