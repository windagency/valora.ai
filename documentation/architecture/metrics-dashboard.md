...
```yaml
Data Collection:
  Source: .valora/sessions/**/review-plan-*.json
  Fields:
...
Formula: early_exit_rate = (reviews_with_early_exit / total_reviews) * 100
...
jq -r '.stages[] | select(.stage == "synthesis") |
  ...
  .valora/sessions/**/review-plan-*.json
```
...
```yaml
Data Collection:
  Source: .valora/sessions/**/plan-*.json
  Fields:
...
Formula: template_rate = (plans_with_template / total_plans) * 100
...
Formula: time_saved = sequential_time_estimate - actual_time
...
```yaml
Data Collection:
  Source: .valora/sessions/**/implement-*.json
  Fields:
...
Formula: error_reduction = (1 - (errors_in_assert / baseline_errors)) * 100
...
Formula: avg_iterations = mean(iteration_count)
...
```yaml
Data Collection:
  Source: .valora/sessions/**/refine-specs-*.json, create-prd-*.json
  Fields:
...
Formula: question_reduction = (1 - (current_avg / baseline_avg)) * 100
...
const input = process.stdin.isTTY ? readFileSync(process.argv[2] || 'metrics.json', 'utf-8') : readFileSync(0, 'utf-8');
  ...
  generate-dashboard:
    ...
    steps:
      ...
      - name: Generate dashboard
        run: |
          pnpm tsx scripts/generate-dashboard.ts metrics.json
...
**File**: `data/templates/WEEKLY_METRICS_REPORT.md`
