...
Verified 2026-05-07 against `data/agents/registry.json`, `packages/*/agents/*.md`, `src/security/`, `src/executor/`, `src/ui/dashboard/`, and `__tests__/regression/`.

- Claims checked: 14 (agent catalogue — 11 agents and their escalation criteria; safety controls table — 7 controls with class names; data inputs table — 6 rows; changelog)
...
- Corrected: 1 — Section 4 agent catalogue regenerated from `scripts/generate-system-card-agents.ts`. The previous catalogue listed ten fabricated agent identifiers (`architect`, `coder`, `context`, `document`, `orchestrator`, `planner`, `reviewer`, `secops`, `tester`, `explorer`) that did not exist in `data/agents/registry.json`; only `asserter` matched. The replacement reflects the eleven real agents (`asserter`, `lead`, `platform-engineer`, `product-manager`, `qa`, `secops-engineer`, four `software-engineer-typescript-*` variants, `ui-ux-designer`) and their declared escalation criteria.
- Unverifiable: 0
