...
Registered in `.valora/hooks.json` (project-level, overrides `data/hooks.default.json`) under `hooks.PreToolUse` with a regex matcher targeting `^run_terminal_cmd$`. This file is separate from `.valora/config.json` so that `valora config setup` does not overwrite hook configuration.
...
The package manager rule is configurable via `.valora/hooks.json` under `enforcement.package_manager`:
...
- `data/hooks.default.json` — Added `hooks` and `enforcement` keys (built-in defaults); overridable via `.valora/hooks.json` (project-level, not overwritten by `config setup`)
