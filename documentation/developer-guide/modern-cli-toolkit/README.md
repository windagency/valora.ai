...
The package manager rule is configurable in `.valora/hooks.json` (project-level override of `data/hooks.default.json`):
...
Set `"enabled": false` to disable the package manager check, or change `"blocked"` / `"replacement"` to match your project convention. This file is separate from `.valora/config.json` so that `valora config setup` does not overwrite hook configuration.
