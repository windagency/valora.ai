...
_(Laisser la diapo s'afficher, marquer une courte pause.)_
...
Pour les plus techniques d'entre vous : c'est une CLI TypeScript, version 2.4.1, disponible sous licence MIT, compatible Node.js 18 et supérieur.
...
_(Lire la citation lentement, en laissant le silence travailler.)_

Je veux commencer par cette phrase, qui est au cœur de la philosophie de VALORA :

> _« L'avenir du développement logiciel n'est pas de remplacer les développeurs, mais d'amplifier leurs capacités avec une collaboration IA intelligente. »_
...
_(Pointer la colonne de gauche, puis la droite.)_
...
**Premier pilier : une CLI puissante.** Vingt-cinq commandes qui couvrent absolument tout le cycle de vie — de la spec initiale jusqu'au déploiement. Vous n'avez plus besoin de jongler entre dix outils différents.
...
En chiffres : onze agents, huit phases, vingt-cinq commandes, trois modes d'exécution, quinze serveurs MCP externes. On va tout parcourir.
...
_(Parcourir la grille en pointant chaque agent.)_
...
_(Parcourir le pipeline de gauche à droite.)_
...
**Phase 8 — Feedback.** On capture ce qui a bien fonctionné, et — nouveauté importante — VALORA consolide automatiquement la mémoire des agents pour les sessions suivantes. `valora feedback` déclenche `valora consolidate` en arrière-plan.
...
_(Pointer chaque couche de haut en bas.)_
...
**La couche Orchestrateur** — le cœur du système. Le Pipeline Executor qui séquence les étapes, le Stage Executor qui gère l'exécution de chaque phase, le Context Manager qui maintient la cohérence entre les appels — et depuis la version 2.4, le **Memory System**, qui permet aux agents d'apprendre et de mémoriser d'une session à l'autre.
...
## Diapo 13 — Mémoire Biologique des Agents

_(Cette diapo est nouvelle depuis la version 2.4 — prenez le temps de l'expliquer, c'est une différenciation forte.)_

Permettez-moi de m'arrêter sur une innovation que je trouve particulièrement intéressante : la **mémoire biologique** des agents.

Jusqu'à présent, chaque session VALORA était une ardoise vierge. Les agents redécouvraient les mêmes patterns, répétaient les mêmes analyses, oubliaient les erreurs passées. C'est le problème fondamental de tous les systèmes IA : l'absence de mémoire persistante.

Nous avons résolu ça en nous inspirant directement de la biologie.

_(Pointer les trois stores.)_

VALORA maintient **trois magasins de mémoire**, stockés dans `.valora/memory/` :

- La **mémoire épisodique** — les observations de session : erreurs détectées, patterns de code remarqués. Elle dure sept jours par défaut.
- La **mémoire sémantique** — les patterns consolidés, le savoir architectural inter-sessions. Trente jours.
- La **mémoire des décisions** — les choix d'architecture persistants, validés par l'équipe. Vingt et un jours.

_(Pointer les mécaniques à droite.)_

Ce qui rend ce système élégant, c'est la **courbe d'Ebbinghaus** — la décroissance exponentielle de l'oubli. Chaque entrée a un "demi-vie" : si elle n'est pas réutilisée, elle s'estompe naturellement. Si elle est accédée souvent, son demi-vie s'allonge. Les erreurs répétées sont mémorisées deux fois plus longtemps que les patterns normaux.

Autre mécanisme intéressant : l'**invalidation par git**. Quand du code change, les entrées mémoire qui référencent ces fichiers sont automatiquement affaiblies. La mémoire reste cohérente avec l'évolution du codebase.

Et tout ça sans aucune dépendance externe — du JSON pur, sans base de données, sans infrastructure.

Pour déclencher une consolidation manuelle : `valora consolidate`. Mais en pratique, ça se fait automatiquement après chaque `valora feedback`.

---

## Diapo 14 — Modes d'Exécution
...
## Diapo 15 — Les 3 Tiers

_(Pointer chaque carte de gauche à droite.)_
...
## Diapo 18 — Les 6 Niveaux de Protection

Six niveaux de protection, tous actifs par défaut.

_(Parcourir les cartes.)_
...
## Diapo 20 — Comment Étendre VALORA

_(Pointer la première colonne.)_
...
_(Pointer la deuxième colonne.)_
...
_(Pointer la troisième colonne.)_
...
## Diapo 21 — Référence des Commandes

Voici un aperçu rapide des vingt-cinq commandes disponibles, organisées selon les trois phases du cycle de vie.

_(Pointer chaque colonne brièvement.)_
...
À droite, la **livraison** — `commit` pour des commits propres, `create-pr` pour la PR, `feedback` pour capturer les résultats — et `consolidate` pour cristalliser la mémoire des agents avant la prochaine session.
...
## Diapo 22 — Démarrage Rapide

Concrètement, à quoi ressemble le premier jour avec VALORA ?

_(Pointer chaque ligne du terminal.)_
...
_(Marquer une pause. Laisser le logo s'afficher.)_
...
_(Pointer la commande d'installation.)_
...
**Durée estimée :** 30 à 40 minutes selon le rythme et les questions intermédiaires (la diapo 13 sur la mémoire est dense — comptez 3 à 4 minutes supplémentaires si l'audience est technique).
...
**Pour les audiences mixtes :** Les diapos 3-4 (le problème) et 15 (les trois tiers) sont les plus accessibles aux non-techniques — insistez dessus, ralentissez. Les diapos 12 (architecture), 13 (mémoire) et 16 (modèles) sont plus denses — vous pouvez les survoler si le temps manque.

**Sur la diapo 13 (mémoire biologique) :** C'est la nouveauté la plus différenciante de la version 2.4. Si l'audience est technique, insistez sur la courbe d'Ebbinghaus et l'invalidation par git — ce sont les deux mécanismes qui la rendent réellement utile. Si l'audience est non-technique, simplifiez : « les agents se souviennent de ce qu'ils ont appris, et oublient naturellement ce qui est devenu obsolète. »

**Transitions à soigner :** Avant chaque diapo divider (3, 5, 7, 9, 11, 14, 17, 19), marquez une pause d'une seconde. Ce sont des respirations dans le récit — laissez-les jouer leur rôle.
