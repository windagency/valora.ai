...
describe('Forbidden Path Validation', () => {
	describe('isForbiddenPath', () => {
		it('should return true for paths starting with .valora/', () => {
			expect(isForbiddenPath('.valora/config.json')).toBe(true);
			expect(isForbiddenPath('.valora/sessions/test')).toBe(true);
			expect(isForbiddenPath('.valora/')).toBe(true);
		});

		it('should return true for paths starting with data/', () => {
			expect(isForbiddenPath('data/agents/registry.json')).toBe(true);
			expect(isForbiddenPath('data/config.default.json')).toBe(true);
			expect(isForbiddenPath('data/')).toBe(true);
		});

		it('should return true for paths with .valora or data in absolute paths', () => {
			expect(isForbiddenPath('/workspaces/project/.valora/config.json')).toBe(true);
			expect(isForbiddenPath('/home/user/project/data/agents/test.md')).toBe(true);
		});

		it('should return true for Windows-style paths', () => {
			expect(isForbiddenPath('.valora\\config.json')).toBe(true);
			expect(isForbiddenPath('data\\agents\\test.md')).toBe(true);
		});

		it('should return false for paths not in forbidden folders', () => {
		...
		it('should return false for paths that contain .valora or data but not as a directory', () => {
			expect(isForbiddenPath('src/valora-utils.ts')).toBe(false);
			expect(isForbiddenPath('docs/.valora-readme.md')).toBe(false);
			expect(isForbiddenPath('src/database.ts')).toBe(false);
		});
	});

	describe('validateNotForbiddenPath', () => {
		it('should throw error for paths in forbidden folders', () => {
			expect(() => validateNotForbiddenPath('.valora/config.json', 'write to')).toThrow(/Cannot write to files/);
			expect(() => validateNotForbiddenPath('data/config.json', 'write to')).toThrow(/Cannot write to files/);
		});

		it('should include the operation type in error message', () => {
			expect(() => validateNotForbiddenPath('.valora/test.md', 'delete')).toThrow(/Cannot delete files/);
			expect(() => validateNotForbiddenPath('data/test.md', 'modify')).toThrow(/Cannot modify files/);
		});

		it('should include the original path in error message', () => {
			expect(() => validateNotForbiddenPath('.valora/sessions/test', 'write to')).toThrow(
				/Path: \.valora\/sessions\/test/
			);
		});
