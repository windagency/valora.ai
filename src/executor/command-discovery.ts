 ...
 */
export const DEFAULT_COMMANDS_DIR = path.join(getPackageDataDir(), 'commands');
...
export function validateCommandsDirectory(commandsDir: string): void {
	...
	// Resolve to absolute path and check if it's within an allowed directory
	const resolvedPath = path.resolve(commandsDir);
	const resolver = getResourceResolver();

	// Ensure the commands directory is within the package data dir or project config dir
	if (!resolver.isAllowedDirectory(resolvedPath)) {
		throw new ValidationError('Commands directory must be within the package data or project config directory', {
			commandsDir,
			resolvedPath
		});
	}
