 ...
 *
 * The Moonshot API is OpenAI-compatible, so this provider talks to it through
 * the OpenAI SDK pointed at Moonshot's base URL. Requires an API key.
 *
...
export class MoonshotProvider extends BaseLLMProvider {
	...
	async complete(options: LLMCompletionOptions): Promise<LLMCompletionResult> {
		...
		if (!checkRateLimit(rateLimitKey, 'llm_api_call')) {
			...
			throw new ProviderError(
				`Moonshot API rate limit exceeded. Try again in ${Math.ceil((status.resetTime - Date.now()) / 1000)} seconds.`,
				{
		...
		const operation = async (): Promise<LLMCompletionResult> => {
			...
			const response = await client.chat.completions.create({
				...
				model: options.model ?? this.getDefaultModel() ?? ModelName.KIMI_K2_6,
				stop: options.stop,
			...
			if (!choice) {
				throw new ProviderError(
					'Moonshot API returned no choices in response',
					{ provider: BuiltinProviders.MOONSHOT, response },
					context
				);
			}
			...
			return {
				...
				tool_calls: choice.message.tool_calls?.map((tc) => {
					...
					try {
						...
						getLogger().debug(
							`Moonshot returned malformed tool call arguments for '${tc.function.name}' — using empty args`,
	...
	override getAlternativeModels(currentModel?: string): string[] {
		const alternatives = getProviderModels(BuiltinProviders.MOONSHOT);
		if (currentModel) {
	...
	override validateModel(modelName: string): Promise<boolean> {
		const knownModels = getProviderModels(BuiltinProviders.MOONSHOT);
		if (knownModels.includes(modelName)) {
			return Promise.resolve(true);
		}
		// Accept anything following Moonshot's kimi-* or moonshot-* naming conventions.
		if (modelName.startsWith('kimi-') || modelName.startsWith('moonshot-')) return Promise.resolve(true);
	...
	private getBaseURL(): string {
		return (this.config['baseUrl'] as string) || DEFAULT_MOONSHOT_BASE_URL;
	}
