...
scripts/generate-commands-registry.sh

# Dry run (preview output without writing files)
scripts/generate-commands-registry.sh --dry-run
```
...
# Preview changes
scripts/generate-commands-registry.sh --dry-run | jq '.commands | keys'
```

## Output

The script generates `data/commands/registry.json` with this structure:
...
Commands are defined in `data/commands/*.md` files with YAML frontmatter containing:
