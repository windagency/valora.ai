...
Generates the `data/prompts/registry.json` file from prompt definition files in the `data/prompts/` directory. This registry catalogs reusable AI prompts with their metadata, dependencies, and requirements.

## Usage

```bash
# Generate registry from prompt files
scripts/generate-prompts-registry.sh

# Dry run (preview output without writing files)
scripts/generate-prompts-registry.sh --dry-run
```

## What it does

1. **Parses YAML frontmatter** from all `.md` files in `data/prompts/` subdirectories (excluding `_template.md` and `_meta/`)
...
# Preview changes
scripts/generate-prompts-registry.sh --dry-run | jq '.prompts | keys'
```

## Output

The script generates `data/prompts/registry.json` with this structure:
...
Prompts are defined in `data/prompts/*/*.md` files with YAML frontmatter containing:
